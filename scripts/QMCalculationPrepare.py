...
class QMCalculationPrepare:
    ...
    def gen_conf_dict(self, obj):
        ...
        if obj.Name in ['pka', 'logk']:
            ...
            resource_conf = {
                ...
                'path_to_input_xyz': '%s/%s/%s/jobs/%s/HA_%s-%s.xyz' %
    ...
    def obj_to_dict(self, obj):
        ...
        if obj.Name in ['pka', 'logk']:
            resource_conf['path_to_input_xyz'] = '%s/%s/%s/jobs/%s/A_%s-%s.xyz' % (self.DjangoHome, self.JobLocation, obj.Name, obj.JobID, obj.Name, obj.JobID)
